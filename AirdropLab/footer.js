...
<div id="supportTicketsModal" class="modal">
    <div class="modal-content modal-lg relative">
        <button onclick="closeSupportTicketsModal()" class="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors bg-slate-800/80 rounded-full w-8 h-8 flex items-center justify-center z-10">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-bold text-white flex items-center gap-3">
                        ...
                        <span id="supportTicketsTitle">Мои обращения</span>
                    </h2>
                    <p class="text-sm text-slate-400 mt-1" id="supportTicketsCount">0 обращений</p>
                </div>
                <button onclick="openNewSupportTicketModal()" class="bg-purple-600 hover:bg-purple-500 px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors flex items-center gap-2">
                    <i class="fas fa-plus"></i> Новое
                </button>
            </div>
        </div>
        <div class="p-4 max-h-[60vh] overflow-y-auto" id="supportTicketsList">
...
<div id="newSupportTicketModal" class="modal">
    <div class="modal-content modal-md p-6 relative">
        <button onclick="closeNewSupportTicketModal()" class="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h2 class="text-xl font-bold text-white">Новое обращение</h2>
                <p class="text-sm text-slate-400">Мы ответим в течение 24 часов</p>
            </div>
        </div>
        
        <form id="newSupportTicketForm" onsubmit="submitNewSupportTicket(event)" class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-slate-300 mb-2">Тема обращения *</label>
                <select id="newTicketCategory" required class="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-white focus:border-purple-500 focus:outline-none">
            ...
            <div>
                ...
                <input type="text" id="newTicketSubject" required placeholder="Краткое описание проблемы" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-white focus:border-purple-500 focus:outline-none">
            </div>
            
            <div>
                <label class="block text-sm font-medium text-slate-300 mb-2">Подробное описание *</label>
                <textarea id="newTicketMessage" required rows="5" placeholder="Опишите вашу проблему подробно..." class="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-white focus:border-purple-500 focus:outline-none resize-none"></textarea>
            </div>
            
            <button type="submit" id="newTicketSubmitBtn" class="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 py-3 rounded-lg text-sm font-bold text-white transition-all hover:scale-[1.02] shadow-lg shadow-purple-500/20">
...
<div id="supportTicketDetailModal" class="modal">
    <div class="modal-content modal-lg p-0 relative">
        <button onclick="closeSupportTicketDetailModal()" class="absolute top-4 right-4 z-10 text-slate-400 hover:text-white transition-colors bg-slate-800/80 rounded-full w-8 h-8 flex items-center justify-center">
            <i class="fas fa-times"></i>
        </button>
        <div class="bg-gradient-to-r from-slate-900 to-slate-800 p-6 border-b border-slate-700">
            <div class="flex items-center gap-3">
                <button onclick="backToSupportTicketsList()" class="flex items-center justify-center w-8 h-8 rounded-lg bg-slate-700 hover:bg-slate-600 text-white transition-colors">
                    <i class="fas fa-arrow-left text-sm"></i>
                </button>
                <div class="flex-1">
                    <h2 class="text-lg font-bold text-white" id="ticketDetailSubject">Обращение</h2>
                    <div class="flex items-center gap-3 mt-1">
                        <span class="text-xs text-slate-400" id="ticketDetailCategory"></span>
                        <span class="text-xs text-slate-500" id="ticketDetailDate"></span>
                    </div>
                </div>
                <span class="px-3 py-1 rounded-full text-xs font-medium" id="ticketDetailStatus"></span>
            </div>
        </div>
        <div class="p-4 max-h-[50vh] overflow-y-auto" id="ticketMessagesList"></div>
        <div class="p-4 border-t border-slate-700 bg-slate-900/50">
            <div class="flex gap-2">
                <input type="text" id="ticketReplyInput" placeholder="Написать ответ..." class="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-white focus:border-purple-500 focus:outline-none" onkeypress="if(event.key==='Enter'){event.preventDefault();sendTicketReply();}">
                <button onclick="sendTicketReply()" class="bg-purple-600 hover:bg-purple-500 px-4 py-2 rounded-lg text-white transition-colors">
...
window.openSupportTicketsModal = function() {
    if (!currentUser) {
    ...
    const titleEl = document.getElementById('supportTicketsTitle');
    if (titleEl) titleEl.textContent = isAdmin ? 'Все обращения' : 'Мои обращения';
    
    renderSupportTicketsList();
    modal.classList.add('active');
};

window.closeSupportTicketsModal = function() {
    const modal = document.getElementById('supportTicketsModal');
    if (modal) modal.classList.remove('active');
};
...
function renderSupportTicketsList() {
    ...
    const countEl = document.getElementById('supportTicketsCount');
    ...
    if (!supportTickets || supportTickets.length === 0) {
        container.innerHTML = `
            <div class="text-center py-12 text-slate-500">
                <i class="fas fa-inbox text-4xl mb-3 opacity-50"></i>
                <p class="mb-4">${isAdmin ? 'Нет обращений' : 'У вас пока нет обращений'}</p>
            </div>
        `;
        if (countEl) countEl.textContent = '0 обращений';
        return;
    }
    
    if (countEl) countEl.textContent = supportTickets.length + ' обращений';
    ...
    container.innerHTML = supportTickets.map(ticket => {
        const isUnread = isAdmin ? !ticket.adminRead : !ticket.userRead;
        const categoryIcon = categoryIcons[ticket.category] || '💬';
        const statusClass = statusClasses[ticket.status] || statusClasses.new;
        
        const createdDate = ticket.createdAt?.toDate ? ticket.createdAt.toDate() : new Date(ticket.createdAt || Date.now());
        const dateStr = formatTimeAgo(createdDate);
        ...
        return `
            <div onclick="openSupportTicketDetail('${ticket.id}')" 
                 class="ticket-item p-4 mb-3 rounded-xl cursor-pointer transition-all ${isUnread ? 'bg-slate-800/50 border border-purple-500/30' : 'bg-slate-800/30 border border-slate-700/50 hover:border-slate-600'}">
                <div class="flex items-start justify-between gap-3 mb-2">
                    <div class="flex items-center gap-2">
                        <span class="text-lg">${categoryIcon}</span>
                        <div>
                            <h4 class="font-medium text-white text-sm">${ticket.subject}</h4>
                            <span class="text-xs text-slate-500">${dateStr}</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        ${isUnread ? '<span class="w-2 h-2 bg-purple-500 rounded-full"></span>' : ''}
                        <span class="px-2 py-0.5 rounded-full text-xs border ${statusClass}">${ticket.status === 'new' ? 'Новое' : ticket.status === 'in_progress' ? 'В работе' : ticket.status === 'resolved' ? 'Решено' : 'Закрыто'}</span>
                    </div>
                </div>
                <p class="text-xs text-slate-400 line-clamp-2">${lastMessage}</p>
            </div>
        `;
    }).join('');
}
...
window.openNewSupportTicketModal = function() {
    ...
    const modal = document.getElementById('newSupportTicketModal');
...
window.submitNewSupportTicket = async function(e) {
    e.preventDefault();
    ...
    const ticketData = {
        ...
        message: document.getElementById('newTicketMessage').value,
        status: 'new',
        userRead: true,
        adminRead: false,
        ...
        messages: [{
            sender: 'user',
            text: document.getElementById('newTicketMessage').value,
    ...
    try {
        await addDoc(collection(db, "supportTickets"), ticketData);
        document.getElementById('newSupportTicketForm').reset();
        closeNewSupportTicketModal();
        footerShowToast('Обращение отправлено!');
    } catch (err) {
        console.error('Error creating ticket:', err);
        footerShowToast('Ошибка при отправке');
    } finally {
        btn.innerHTML = originalText;
        btn.disabled = false;
    }
...
window.openSupportTicketDetail = function(ticketId) {
    ...
    const categoryNames = {
        ...
        project: '📋 Вопрос о проекте',
        suggestion: '💡 Предложение',
        partnership: '🤝 Партнёрство',
        other: '💬 Другое'
    };
    
    document.getElementById('ticketDetailCategory').textContent = categoryNames[ticket.category] || ticket.category;
    
    const createdDate = ticket.createdAt?.toDate ? ticket.createdAt.toDate() : new Date(ticket.createdAt || Date.now());
    document.getElementById('ticketDetailDate').textContent = formatTimeAgo(createdDate);
    ...
    statusEl.className = 'px-3 py-1 rounded-full text-xs font-medium border ' + (statusClasses[ticket.status] || statusClasses.new);
    statusEl.textContent = statusNames[ticket.status] || ticket.status;
    
    subscribeToTicketUpdates(ticketId);
    closeSupportTicketsModal();
    modal.classList.add('active');
...
function subscribeToTicketUpdates(ticketId) {
    if (ticketDetailUnsubscribe) {
        ticketDetailUnsubscribe();
        ticketDetailUnsubscribe = null;
    }
    
    if (typeof onSnapshot === 'undefined') return;
    
    ticketDetailUnsubscribe = onSnapshot(doc(db, "supportTickets", ticketId), (snap) => {
        if (!snap.exists()) return;
        const ticket = snap.data();
        ticket.id = snap.id;
        
        const idx = supportTickets.findIndex(t => t.id === ticketId);
        if (idx !== -1) supportTickets[idx] = ticket;
...
function renderTicketMessages(ticket) {
    const container = document.getElementById('ticketMessagesList');
    if (!container) return;
    ...
    container.innerHTML = messages.map(msg => {
        ...
        const senderName = isUser ? (ticket.userName || 'Пользователь') : 'Поддержка';
        const msgTime = msg.timestamp ? formatTimeAgo(new Date(msg.timestamp)) : '';
        
        return `
            <div class="ticket-message mb-4 ${isUser ? 'text-right' : 'text-left'}">
                <div class="inline-block max-w-[80%] ${isUser ? 'bg-purple-600/20 border-purple-500/30' : 'bg-slate-700/50 border-slate-600/50'} rounded-xl px-4 py-3 border">
                    <div class="text-xs text-slate-400 mb-1">${senderName}</div>
                    <div class="text-sm text-white">${msg.text}</div>
                    <div class="text-xs text-slate-500 mt-1">${msgTime}</div>
                </div>
            </div>
        `;
    }).join('');
...
async function markTicketAsRead(ticketId) {
    if (!currentUser || typeof updateDoc === 'undefined') return;
    
    const isAdmin = currentUser.uid === "SAkz4mdW9reDaIsvqigCNZhEKJR2";
    const updateField = isAdmin ? 'adminRead' : 'userRead';
...
window.sendTicketReply = async function() {
    ...
    try {
        ...
        if (!ticketSnap.exists()) {
            footerShowToast('Обращение не найдено');
            return;
        }
        ...
        await updateDoc(ticketRef, {
            ...
            status: isAdmin ? 'in_progress' : ticketData.status,
            updatedAt: new Date()
        });
...
window.closeSupportTicketDetailModal = function() {
    if (ticketDetailUnsubscribe) {
        ticketDetailUnsubscribe();
        ticketDetailUnsubscribe = null;
    }
...
window.backToSupportTicketsList = function() {
...
// Глобальная функция для инициализации прослушивания
window.initSupportTicketsListener = function(uid) {
    if (supportTicketsUnsubscribe) {
        supportTicketsUnsubscribe();
    }
    ...
    try {
        ...
        supportTicketsUnsubscribe = onSnapshot(q, (snapshot) => {
            supportTickets = [];
            let unreadCount = 0;
            ...
            snapshot.forEach((doc) => {
                ...
                if (isAdmin || data.userId === uid) {
                    data.id = doc.id;
                    const isUnread = isAdmin ? !data.adminRead : !data.userRead;
            ...
            const ticketsModal = document.getElementById('supportTicketsModal');
            if (ticketsModal && ticketsModal.classList.contains('active')) {
                renderSupportTicketsList();
            }
        });
    } catch (e) {
        console.error("Error init support tickets listener:", e);
    }
};
